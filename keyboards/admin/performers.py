...
def get_performer_detail_keyboard(user_id: int, *, status_value: str) -> InlineKeyboardMarkup:
    ...
    if status_value == "ban":
        ...
        rows.append([InlineKeyboardButton(text="⛔ Заблокировать", callback_data=f"admin_performer_ban:{user_id}")])
        toggle_title = "👤 Сделать user" if status_value == "admin" else "⭐ Сделать admin"
